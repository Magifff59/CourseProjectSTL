...
// макрос для скидання кольору у консолі
...
std::string Logger::getColor(LogLevel level) const {
    switch (level) {
        ...
        case LogLevel::FATAL:   return "\033[1;31;43m"; // червоний на жовтому фоні
...
// форматований вивід одного запису
...
void Logger::addLog(LogLevel level, const std::string& msg, std::string funcName) {
    // emplace_back для ефективности (створює об'єкт прямо в контейнері без копіювання)
...
void Logger::loadFromFile(const std::string& filename) {
    ...
    while (std::getline(inFile, line)) {
        ...
        if (firstBracketOpen != std::string::npos && firstBracketClose != std::string::npos &&
            ...
            LogEntry entry(lvl, message); // виклик конструктору (поставить поточний час)
            entry.timestamp = logTime;    // мануально змінюється час на той, що дістано з файлу
...
void Logger::removeLogsByLevel(LogLevel level) {
    ...
    //  алгоритм для "відсування" непотрібних елементів у кінець і потім видаляє їх
...
// --- МЕТОДИ ВАЛІДАЦІЇ ВВОДУ ---

// перевірка символів (y/n)
char Logger::getValidChar(const std::string& prompt, const std::string& validOptions) {
    ...
    while (true) {
        ...
        // очищення буферу від усього залишку після першого символу
        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');

        if (validOptions.find(input) != std::string::npos) return input; // якщо символ допустимий - повертає його
...
// перевірка чисел (діапазон від min до max)
